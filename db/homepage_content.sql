begin;
-- No editorial/settings table existed when inspected. One singleton, no content duplication.
...
grant update (eyebrow, title, subtitle, primary_label, secondary_label, banner, figures_title, profiles_label, positions_label, events_label, candidates_title, primaries_title, primaries_description, methodology) on public.homepage_content to authenticated;
create policy "Public reads homepage" on public.homepage_content for select to anon, authenticated using (true);
create policy "Administrators edit homepage" on public.homepage_content for update to authenticated
...
-- Reuse the existing server-side timestamp trigger for optimistic concurrency.
create trigger homepage_updated_at before update on public.homepage_content
 for each row execute function public.touch_content_updated_at();
insert into public.homepage_content (id) values ('home');
comment on table public.homepage_content is 'Public homepage copy. Manual administration only; automated collection must not write here.';
commit;
